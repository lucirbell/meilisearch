...
get_latest() {
    temp_file='temp_file' # temp_file needed because the grep would start before the download is over
        ...
        curl -s 'https://api.github.com/repos/lucirbell/meilisearch/releases' > "$temp_file" || return 1
    else
        curl -H "Authorization: token $GITHUB_PAT" -s 'https://api.github.com/repos/lucirbell/meilisearch/releases' > "$temp_file" || return 1
    fi
...
if [ "$latest" = '' ]; then
    ...
    echo 'Please let us know about this issue: https://github.com/lucirbell/meilisearch/issues/new/choose'
    exit 1
fi
...
link="https://github.com/lucirbell/meilisearch/releases/download/$latest/$release_file"
curl -OL "$link"
